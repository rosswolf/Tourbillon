...
static func apply_source_modifiers(packet: DamagePacket) -> DamagePacket:
	...
	# Check if source is a combatant
	var combatant = packet.source as ICombatant
	if combatant:
		# Apply damage multipliers
		packet.multipliers *= combatant.get_damage_multiplier()
		
		# Apply flat damage bonuses
		packet.flat_bonuses += combatant.get_damage_bonus()
		
		# Check for critical strike
		if packet.can_crit and randf() < combatant.get_crit_chance():
...
static func apply_global_modifiers(packet: DamagePacket) -> DamagePacket:
	...
	# Will be implemented when GlobalGameManager has damage modifier support
	
	return packet

# Apply target's defenses (shields, armor)
static func apply_defenses(damage: int, packet: DamagePacket, target: IDamageable) -> int:
...
static func apply_shields(damage: int, shield_amount: int, packet: DamagePacket, target: IDamageable) -> int:
	...
	if shield_amount >= damage:
		...
		if packet.has_property(DamageProperty.Type.POP):
...
static func apply_vulnerabilities(damage: int, packet: DamagePacket, target: IDamageable) -> int:
	var modifier = target.get_damage_modifier(packet.damage_type)
	return int(damage * modifier)

# Process chain damage
static func process_chain_damage(packet: DamagePacket, initial_target: IDamageable) -> void:
...
static func find_chain_target(from_target: IDamageable, exclude_list: Array) -> IDamageable:
	# This would use the targeting system to find adjacent/nearby targets
	# Placeholder implementation
	return null

# Process area damage
static func process_area_damage(packet: DamagePacket, center_position: Vector2, radius: int) -> void:
	if packet.splash_radius <= 0:
		return
	
	# Get all entities in radius
	var targets: Array[IDamageable] = []  # Would get from positioning system
	
	for target in targets:
		var targetable = target as ITargetable
		if not targetable:
			continue
			
		var distance = (targetable.get_position() - center_position).length()
		if distance <= radius:
