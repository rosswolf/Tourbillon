...
func set_hovered(instance_id: String) -> void:
	if __hovered != "":
		print("[DEBUG] hover wasnt unset") #This can legit happen for 2 adjacent bodies

	__hovered = instance_id

func clear_hovered_known(instance_id: String) -> void:
	if __hovered != instance_id:
		print("[DEBUG] __hovered doesn't match " + __hovered + " " + instance_id)
	clear_hovered_force()
...
func activate_selected_onto_hovered(last_pos: Vector2) -> void:
	if __selected == "":
		...
		print("[DEBUG] execute: " + __selected + " " + __hovered)
