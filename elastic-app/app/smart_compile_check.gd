...
func _init():
	...
	if errors_found > 0:
		print("[DEBUG] [COMPILE CHECK] ❌ ERRORS FOUND:")
		print("[DEBUG]   Compilation failed with %d errors" % errors_found)
	else:
		print("[DEBUG] [COMPILE CHECK] ✅ All scripts compile successfully!")
...
func build_type_registry():
	...
	if not dir:
		print("[DEBUG] ❌ Failed to open project directory")
		return
	...
	if VERBOSE:
		...
		for enum_path in enum_registry:
			print("[DEBUG]     - %s: %s" % [enum_path, enum_registry[enum_path]])
		print("[DEBUG] \n  Classes found:")
		for class_path in class_registry:
			print("[DEBUG]     - %s: %s" % [class_path, class_registry[class_path]])
...
func check_autoloads_smart():
	...
	if project.load("res://project.godot") != OK:
		print("[DEBUG] ❌ Failed to load project.godot")
		errors_found += 1
		return
	...
	print("[DEBUG]   Found %d autoloads to check" % autoloads.size())
...
func analyze_script_dependencies(script_path: String) -> Array:
	...
	var autoload_pattern = "\\b(StaticData|GlobalSignals|GlobalSelectionManager|" + \
		"GlobalGameManager|TimerService|PreloadScenes|GlobalUtilities|UidManager|UiController)\\b"
	autoload_regex.compile(autoload_pattern)
				...
				if current.processing:
					...
					for item in stack:
						if item.name == current.name and item != current:
							print("[DEBUG]   ⚠️ Circular dependency detected involving: %s" % current.name)
							has_cycle = true
							break
...
func check_single_autoload(name: String, path: String):
	...
	if not path.ends_with(".gd"):
		if VERBOSE:
			print("[DEBUG]   ⏩ Skipping non-script autoload: %s" % name)
		return

	var script_path = path.strip_edges()
	if not FileAccess.file_exists(script_path):
		print("[DEBUG]   ❌ Autoload script not found: %s -> %s" % [name, script_path])
		errors_found += 1
		return
	...
	if script == null:
		print("[DEBUG]   ❌ Failed to load autoload script: %s" % name)
		errors_found += 1
		return

	# Check if it's a valid script
	if not script is GDScript:
		print("[DEBUG]   ❌ Invalid script type for autoload: %s" % name)
		errors_found += 1
		return
...
func check_script_file(script_path: String):
	...
	if not script is GDScript:
		if VERBOSE:
			print("[DEBUG]   ⏩ Not a GDScript file: %s" % script_path)
		return
...
func check_script_syntax(script_path: String):
	...
	var file = FileAccess.open(script_path, FileAccess.READ)
	if not file:
		print("[DEBUG]   ❌ Cannot read script: %s" % script_path)
		errors_found += 1
		return
...
func check_script_source(script_path: String, source: String):
	...
	for pattern_data in suspicious_patterns:
		...
		if regex.compile(pattern_data.pattern) == OK:
			...
			for match in matches:
				...
				print("[DEBUG]   ⚠️ %s:%d - %s" % [script_path.get_file(), line_num, pattern_data.message])
				warnings_found += 1
				found_issues = true
	...
	for i in range(lines.size()):
		if func_regex.search(lines[i]):
			print("[DEBUG]   ❌ %s:%d - Missing ':' after function definition" % [script_path.get_file(), i + 1])
			errors_found += 1
			found_issues = true
...
func print_results():
	# Print final results
	print("\n============================================================")
	print("[DEBUG] 📊 Compilation Check Results")
	print("============================================================")

	print("[DEBUG] Files checked: %d" % checked_files)
	print("[DEBUG] Files exempted: %d" % exempted_files)
	print("[DEBUG] Errors found: %d" % errors_found)
	print("[DEBUG] Warnings found: %d" % warnings_found)
