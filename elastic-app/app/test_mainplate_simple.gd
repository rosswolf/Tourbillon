...
func _ready():
	...
	# Initialize game
	print("1. Setting hero template...")
	GlobalGameManager.hero_template_id = "champion"
	
	print("2. Emitting game start signal...")
	...
	for prop in GlobalGameManager.get_property_list():
		if prop.name == "mainplate":
			...
			print("   Found mainplate property: ", value != null)
			if value != null:
				print("   - Mainplate exists!")
			break
	...
	for node in get_tree().get_nodes_in_group("*"):
		...
		if "mainplate" in node_name.to_lower() or "mainplate" in script_path.to_lower():
			print("   Found mainplate-related node: ", node_name, " [", script_path, "]")
			
			# Check for gear_slots property
			if node.has("gear_slots"):
				var slots = node.get("gear_slots")
				if slots is Dictionary:
					print("     - Has gear_slots dictionary with ", slots.size(), " entries")
					...
					for pos in slots:
						...
						if slot.has("is_active_slot"):
							if slot.is_active_slot:
								...
								inactive += 1
					print("     - Active slots: ", active, ", Inactive slots: ", inactive)
			
			count += 1
	
	if count == 0:
		print("   No mainplate nodes found in scene")
	...
	for sig in GlobalSignals.get_signal_list():
		if sig.name == "ui_execute_selected_onto_hovered":
			...
			print("   ui_execute_selected_onto_hovered has ", connections.size(), " connections")
