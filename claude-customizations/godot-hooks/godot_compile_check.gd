...
func _init() -> void:
	print("[DEBUG] [COMPILE CHECK] Starting comprehensive compilation check...")
	...
	if errors_found.size() > 0 or warnings_found.size() > 0:
		...
		if warnings_found.size() > 0:
			print("[DEBUG] [COMPILE CHECK] ⚠️ WARNINGS:")
			for warning in warnings_found:
				print("[DEBUG]   " + warning)

		print("[DEBUG] ")
		print("[DEBUG] [COMPILE CHECK] Active checks:")
		...
		if errors_found.size() > 0:
			...
			print("[DEBUG] [COMPILE CHECK] ⚠️ Warnings found but no errors - proceeding")
			quit(0)
	else:
		print("[DEBUG] [COMPILE CHECK] ✅ All scripts compile successfully!")
		print("[DEBUG] [COMPILE CHECK] ✅ No private variable access violations found!")
		print("[DEBUG] [COMPILE CHECK] ✅ All type annotations present!")
		print("[DEBUG] ")
		print("[DEBUG] [COMPILE CHECK] Active checks:")
...
func check_autoloads() -> void:
	print("[DEBUG] [COMPILE CHECK] Checking autoloads...")
...
func check_all_scripts() -> void:
	print("[DEBUG] [COMPILE CHECK] Checking all scripts...")
...
func _check_script(script_path: String) -> void:
	...
	if _is_file_exempt(script_path, "fully_exempt_files"):
		print("[DEBUG]   [EXEMPT] Skipping all checks for: " + script_path)
		return
